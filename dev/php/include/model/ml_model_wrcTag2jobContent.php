 ...
 * 
 */
class ml_model_wrcTag2jobContent extends Lib_datamodel_db 
{
    ...
    function __construct($dataDefine = '')
    {
        ...
        parent::__construct('wrc_tag2jobContent' , $db_config['wrc_tag2jobContent']);
    }
    ...
    function getByTagid($aTagid)
    {
        if(!$this->init_db($uid , self::DB_SLAVE))
            return false;

        $tagid_min = min($aTagid);
        $tagid_max = max($aTagid);
        $sql = 'select * from '.$this->table.' where tagid_1 = '.$tagid_min.' and tagid_2 = '.$tagid_max.' and status='.self::STATUS_NORMAL;
        return $this->fetch_row($sql);
    }
